...
logger = pl_loggers.TensorBoardLogger(
    save_dir=data_path, name='logs/mberta/', 
    version=f"mberta_no_val_24"
)
...
checkpoint_callback = ModelCheckpoint(
    ...
    mode="max",
    dirpath=data_path +"logs/mberta/",
    filename="mberta_train_24-{epoch:02d}-{train/F1Score:.2f}",
    save_weights_only=False,
)

test_writer = CustomWriter(
    write_interval='epoch',
    output_file=data_path + 'outputs/mberta_test_preds.pkl'
)

expert_writer = CustomWriter(
    write_interval='epoch',
    output_file=data_path + 'outputs/mberta_expert_preds.pkl'
)
