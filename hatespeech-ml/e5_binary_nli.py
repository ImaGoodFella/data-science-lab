...
train_loader = get_dataloader(
    ...
    use_cache=True, 
    nickname='e5_nli_binary', 
    comment_column='kommentar_original',
    label_column='hatespeech',
    shuffle=True,
    additional_column='toxic_new',
    additional_hypothesis='This text contains non-targeted toxicity'
)
...
logger = pl_loggers.TensorBoardLogger(
    ...
    version=f"e5_nli_binary_no_val_16"
)
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename="e5_nli_binary_train_16-{epoch:02d}-{train/F1Score:.2f}",
    save_weights_only=False,
)

test_writer = CustomWriter(
    write_interval='epoch',
    output_file=data_path + 'outputs/e5_nli_binary_test_preds_16.pkl'
)

expert_writer = CustomWriter(
    write_interval='epoch',
    output_file=data_path + 'outputs/e5_nli_binary_expert_preds_16.pkl'
)
