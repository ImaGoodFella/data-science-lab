...
def prepare_data(df, is_test = False, comment_column='kommentar_original', label_column = 'hatespeech', hypothesis_pos = "This text contains hatespeech"):
    
    df_ret = pd.DataFrame()
    df_ret['label'] = df[label_column].astype(int)
    df_ret['hypothesis'] = hypothesis_pos
    df_ret['premise'] = df[comment_column]

    return df_ret

def prepare_all_nli_data(df, is_test = False, comment_column='kommentar_original'):
...
def get_dataloader(model_name, model_nickname, data_path, use_cache=True, batch_size=32, shuffle=True, dataset_name='new_all_DEFR_comments_27062022.csv', nickname='', is_test=False, comment_column='kommentar', label_column='hatespeech', additional_column=None, additional_hypothesis=None, add_nli=False):
    ...
    if use_cache and os.path.exists(cache_file):
        ...
        df_processed = prepare_data(df, is_test, comment_column=comment_column, label_column=label_column)

        if additional_column is not None:
            df_additional = prepare_data(df, is_test, comment_column=comment_column, label_column=additional_column, hypothesis_pos=additional_hypothesis)
            df_processed = pd.concat([df_processed, df_additional], axis=0)
        
        if add_nli:
            df_filtered = df[df[label_column].astype(bool)]
            df_add_nli = prepare_all_nli_data(df_filtered, is_test, comment_column=comment_column)
            df_processed = pd.concat([df_processed, df_add_nli], axis=0)
        ...
        if use_cache and not os.path.exists(cache_path): os.makedirs(cache_path)
