...
logger = pl_loggers.TensorBoardLogger(
    ...
    version=f"e5_sophia_no_val_{batch_size}"
)
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename=f"e5_sophia_train_{batch_size}"+"-{epoch:02d}-{train/F1Score:.2f}",
    save_weights_only=False,
)

test_writer = CustomWriter(
    write_interval='epoch',
    output_file=data_path + f'outputs/e5_sophia_test_preds_{batch_size}.pkl'
)

expert_writer = CustomWriter(
    write_interval='epoch',
    output_file=data_path + f'outputs/e5_sophia_expert_preds_{batch_size}.pkl'
)
